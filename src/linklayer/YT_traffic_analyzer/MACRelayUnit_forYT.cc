...
Define_Module(MACRelayUnit_forYT);

MACRelayUnit_forYT::MACRelayUnit_forYT()
{
    addressTable = NULL;
}

void MACRelayUnit_forYT::initialize(int stage)
{
    if (stage == 0)
    {
        // number of ports
...
void MACRelayUnit_forYT::handleMessage(cMessage *msg)
{
    if (!isOperational)
    {
...
void MACRelayUnit_forYT::handleAndDispatchFrame(EtherFrame *frame)
{
...
void MACRelayUnit_forYT::broadcastFrame(EtherFrame *frame, int inputport)
{
...
void MACRelayUnit_forYT::start()
{
    addressTable->clearTable();
    isOperational = true;
}

void MACRelayUnit_forYT::stop()
{
    addressTable->clearTable();
    isOperational = false;
}

bool MACRelayUnit_forYT::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void MACRelayUnit_forYT::finish()
{
