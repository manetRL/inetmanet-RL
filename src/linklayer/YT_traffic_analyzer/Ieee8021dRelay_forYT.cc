...
#include "Ieee8021dRelay_forYT.h"
...
Define_Module(Ieee8021dRelay_forYT);

Ieee8021dRelay_forYT::Ieee8021dRelay_forYT()
{
    ifTable = NULL;
    macTable = NULL;
    ie = NULL;
...
void Ieee8021dRelay_forYT::initialize(int stage)
{
...
void Ieee8021dRelay_forYT::handleMessage(cMessage * msg)
{
    if (!isOperational)
    {
    ...
    if (!msg->isSelfMessage())
    {
        ...
        else if (strcmp(msg->getArrivalGate()->getName(), "ifIn") == 0)
        {
            ...
            cMessage *copy_forAnalyzer = msg->dup();
            cModule *mainModule = getParentModule()->getSubmodule("traffic_analyzer",0);
            YT_traffic_analyzer *traff_an = check_and_cast<YT_traffic_analyzer *>(mainModule);
            sendDirect(copy_forAnalyzer,traff_an, "traffic_In");
...
void Ieee8021dRelay_forYT::broadcast(EtherFrame * frame)
{
...
void Ieee8021dRelay_forYT::handleAndDispatchFrame(EtherFrame * frame)
{
...
void Ieee8021dRelay_forYT::dispatch(EtherFrame * frame, unsigned int portNum)
{
...
void Ieee8021dRelay_forYT::learn(EtherFrame * frame)
{
    int arrivalGate = frame->getArrivalGate()->getIndex();
...
void Ieee8021dRelay_forYT::dispatchBPDU(BPDU * bpdu)
{
...
void Ieee8021dRelay_forYT::deliverBPDU(EtherFrame * frame)
{
...
Ieee8021dInterfaceData * Ieee8021dRelay_forYT::getPortInterfaceData(unsigned int portNum)
{
    if (isStpAware)
    {
...
void Ieee8021dRelay_forYT::start()
{
    isOperational = true;
...
void Ieee8021dRelay_forYT::stop()
{
    isOperational = false;
...
InterfaceEntry * Ieee8021dRelay_forYT::chooseInterface()
{
...
void Ieee8021dRelay_forYT::finish()
{
...
bool Ieee8021dRelay_forYT::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
